<template>
  <div class="hxr-main">
    <my-bread level1="菜谱管理" level2="菜谱列表" />
    ...
    <el-row class="hxr-RowChange" :gutter="20">
      <el-col :span="24" class="hxr-btnGroup">
        <div class="hxr-bgCell">
          <el-button @click="addMenu()" size="small" type="success">添加菜谱</el-button>
        </div>
        <div class="hxr-bgCell">
          <el-button @click="postExcel()" size="small" type="info">批量导入</el-button>
        </div>
        <div class="hxr-bgCell">
          <el-button @click="getExcel()" size="small" type="info">批量导出</el-button>
        </div>
        <div class="hxr-bgCell">
...
export default {
  ...
  methods: {
    ...
    addMenu() {
      this.$router.push({ path: "/menu/menuAdd" });
    },
    postExcel() {
    ...
    delMenu() {
      if (this.$refs.multipleTable.selection.length > 0) {
        ...
        delMenuList(dataModule).then((response) => {
          if (response.status === 200) {
